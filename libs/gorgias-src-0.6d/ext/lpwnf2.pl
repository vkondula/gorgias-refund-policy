...
% attacks0(?QH, ?LT, +Argument, +Culprit, ?CounterArgument)
...
attacks0('ASS', 'D', ass(L), _, CA) :-
	...
	abducible(NL, Conditions),

	resolve(Conditions, Resolvent),
...
%%% For Negation As Failure (Gorgias)


attacks0('NAF', _, naf_ass(L), _, CA) :-

	resolveone(['RES','NAF','BUILT_IN'], L, [], Resolvent),
