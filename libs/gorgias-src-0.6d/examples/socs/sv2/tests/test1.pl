...
test__heading('TEST 1 -- Reactivity using Preference Reasoning II').

test__comment(0, 'The computee rejects the offer to share a taxi. There is a single taxi available and the computee knows (via its temporal reasoning capability) that Francisco has enough cash.').

test__comment(1, 'Now, note that the reaction to reject the offer has been added to the list of actions. Also note that the punctual profile selects the only action available since it is urgent and demotes the rest of the transitions.').

test__comment(2, 'Note that the executed action have been added to KB 0.').


:- self__logging_set(on).

:- self__commit(step('PO', [obs(single_taxi_available),obs(neg(out_of_cash)),obs(c1,(request(c0,share_taxi_p),0),1)])).
...
:- start.
:- self__commit(step('AE',([((response(c1,share_taxi_p,reject),3),[],[],[])], []))).
:- start.
